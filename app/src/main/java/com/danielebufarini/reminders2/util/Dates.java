...
public class Dates {
    private static final SimpleDateFormat DATE_FORMAT_WO_YEAR = new SimpleDateFormat("EEEE, MMM d", Locale.US);
    private static final SimpleDateFormat DATE_FORMAT_W_YEAR = new SimpleDateFormat("EEEE, MMM d, y", Locale.US);
    ...
    public static List<GTask> sortTasksByDate(final List<GTask> tasks) {
        ...
        for (GTask task : tasks) {
            ...
            if (Dates.isToday(cal) || Dates.isTomorrow(cal))
                firstTwoDays.add(task);
            else if (Dates.isNextWeek(cal))
                nextWeek.add(task);
            else if (Dates.isInThePast2(cal))
                inThePast.add(task);
            else if (Dates.isInTheFuture(cal))
